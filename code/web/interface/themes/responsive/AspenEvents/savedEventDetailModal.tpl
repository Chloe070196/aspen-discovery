...
{if !empty($loggedIn)}
	...
	{include file='AspenEvents/seats.tpl'  numberOfSeats="{$event.numberOfSeats}" availableSeats="{$event.availableSeats}" isEventFull="{$event.isEventFull}" userOnWaitingList="{$event.userOnWaitingList}" userWaitingListPosition="{$event.userWaitingListPosition}" userIsRegistered="{$event.isRegistered}" isWaitingListFull="{$event.waitingListFull}"}
	<section class="well">

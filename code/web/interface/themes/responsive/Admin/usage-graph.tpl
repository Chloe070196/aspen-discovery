{strip}
	<div id="main-content" class="col-sm-12">
		...
		{if !empty($showCSVExportButton)}
			<div>
				<a id="UsageGraphExport" class="btn btn-sm btn-default" href="/{$section}/AJAX?method=exportUsageData&stat={$stat}{if !empty($profileName)}&profileName={$profileName}{/if}{if !empty($instance)}&instance={$instance}{/if}{if !empty($subSection)}&subSection={$subSection}{/if}">{translate text='Export To CSV' isAdminFacing=true}</a>
